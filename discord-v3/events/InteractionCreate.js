...
export async function execute(client, interaction){
    ...
    if (interaction.isChatInputCommand()) {
        ...
        const command = client.commands.data[commandName];
        ...
        if (buttonInfo[0] == 'pronouns') {
            ...
            if (member.roles.cache.some(r => r.id === notify_roles.buttons[which].roleID)) {
                member.roles.remove(role);
                interaction.editReply({ content: `Removing the ${client.functions.ucwords(which)} role.`, flags: MessageFlags.Ephemeral });
            }
            else {
                member.roles.add(role);
                interaction.editReply({ content: `Adding the ${client.functions.ucwords(which)} role.`, flags: MessageFlags.Ephemeral });
            }
        }
        ...
        const command = client.commands.data[interaction.commandName];
