...
module.exports = {
	async function(type, data, client, reset = false) {
		...
		if (type == 'addons') {
			if (reset) { client.settings.addons = new Array(); }
			if (!('settings' in client)) { client.settings = []; }
			...
			if (reset) { client.overlay = new Array(); }
			if (!('overlay' in client)) { client.overlay = []; }
			...
			if (reset) { client.reactwords = new Array(); }
			if (!('reactwords' in client)) { client.reactwords = {}; }
			...
			if (reset) { client.timers = new Array(); }
			if (!('timers' in client)) { client.timers = []; }
