					...
					.then(function(response) {
						...
						if (resData.status === 'success') {
							...
							if (resData.err_msg === 'missing_authorization') {
								data.errorMsg.handle(channel, client, 'bonk', 'Authorization issue');
							}
							else {
								data.errorMsg.handle(channel, client, 'bonk', 'Failed response');
							}
						}
					})
					.catch(function() {
						data.errorMsg.handle(channel, client, 'bonk', 'Issue while handling command');
					})
					.finally(function() {
						if (content !== '') {
